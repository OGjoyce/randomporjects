...
void initialize(){
	//TODO: Implemente el metodo initialize reservando memoria para la tabla de simbolos

	symtab_s =(char **)malloc(sizeof(char * )*TABLE_LEN);
	symtab_n =(long *)malloc(TABLE_LEN * sizeof(long));
	int i =0;
	for (i; i < TABLE_LEN; ++i)
	{
		symtab_s[i] =(char*)malloc(MAX_LABEL_LEN+1);
...
void release(){
	//TODO:Implemente el memtodo release liberando la memoria reservada por initialize
		int i = 0;
	for (i; i < TABLE_LEN; ++i)
	{
		free((char*)symtab_s[i]);
...
void add(char *string, long dir){
	...
	//la posicion que corresponda'
		...
		strcpy(symtab_s[label_count], string);
